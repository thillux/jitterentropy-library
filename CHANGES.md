3.7.1-prerelease
 * Health tests: the recovery loop of the RCT with memory keeps testing the output block it recovers, and samples only the noise source the current collection measures - during the NTG.1 startup stages it sampled both
 * Jitter RNG core: a clock that stops advancing after the startup ends the collection with JENT_ERR_RCT_PERMANENT in every mode instead of looping forever
 * Jitter RNG core: an out-of-range RCT-with-memory window stops the output in every mode, not only under FIPS. Reachable only with JENT_MAX_OSR raised far above its default
 * Jitter RNG core: without a CSPRNG an instance has no UUID instead of the nil UUID every instance shared. The uuid status field is empty and JENT_IOCUUID returns ENODATA
 * Documentation: state the thread safety contract in jitterentropy.h and the man page, document jent_status, jent_uuid, jent_secure_memory_supported and jent_entropy_set_notime_cpu, and state that the first initialization attempt closes the configuration window
 * Jitter RNG core: the shared library no longer exports the internal jent_gcd_* functions on macOS and Windows
 * Tests: new exported-symbols test asserting that the shared library exports exactly the functions of version.lds
 * Linux kernel: new max_instances module parameter (default 256, 0 for unlimited) caps the /dev/jitterentropy instances open at once; CAP_SYS_RESOURCE is exempt. Kernel allocations are charged to the caller's memory cgroup
 * Linux kernel: new max_memsize module parameter bounds the memory access region of an instance, including its growth on health test recovery
 * Linux kernel: the per-instance procfs files, statistics and hwrng_status are readable by root only
 * Linux kernel: the debugfs raw noise recording yields and checks for signals per measurement and re-primes after a reschedule; JENT_IOCLOOPCNT is bounded by JENT_LOOPCNT_MAX
 * Health tests: a health test reallocation primes the RCT with memory at the replacement's own cutoff
 * Health tests: the reallocation grows the memory region unless the caller chose its size; every reallocated collector was taken as pinned
 * Jitter RNG core: jent_read_entropy_safe() reports a recovery it gave up on without generating another block, and rejects a NULL collector with JENT_ERR_EINVAL
 * Jitter RNG core: jent_status reports no memory size for an instance without memory access, and flags a document that exactly fills the buffer as complete
 * Jitter RNG core: the XDRBG state and the block returned to the caller no longer pass through the stack, and every entry point that runs the noise source wipes the stack it used before returning (hosted builds). A hosted caller needs some 4.5 kB of stack for these calls, 8 kB for the first allocation on Windows
 * Tests: new unit-stack-residue checking that no time stamp or collector state is left on the stack
 * Jitter RNG core: a forced internal timer startup that fails no longer commits the process to the internal timer
 * Build: MSVC builds of the noise source are unoptimized by #pragma optimize and noinline helpers rather than by flag order; /GL, /LTCG and interprocedural optimization are refused
 * Health tests: the cutoff tables cover oversampling rates up to 64, so JENT_MAX_OSR can be raised to 64 (it stays 20). A rate outside the tables fails the allocation instead of using the cutoffs of another rate
 * Jitter RNG core: new JENT_MAX_OSR_FIPS and JENT_MAX_OSR_NTG1 (both 20) bound the oversampling rate in the compliance modes independently of JENT_MAX_OSR
 * Linux kernel: an osr module parameter above the maximum fails the module load with EINVAL instead of failing the startup
 * Jitter RNG core: the SHAKE-256 known answer test runs as part of the conditioning self test
 * Tests: extractlsb creates its output with mode 0644, jitterentropy-cpuinfo bounds CPU list ranges, and the unbuildable recording_runtime_kernelspace/attic is removed
 * Tests: Android (Kotlin, Compose) and iOS (SwiftUI) example apps under tests/android and tests/ios, replacing arch/android
 * Jitter RNG core: the memory access region is no longer secure memory - it holds no collector state and locking it exceeded common lock quotas, failing compliance-mode collectors on Android, Windows and in containers. It stays zeroed, guard-paged, excluded from core dumps and wiped. The hash state is part of struct rand_data, so a collector locks one page; jent_sha3_alloc and jent_sha3_dealloc are removed
 * Documentation: JENT_FORCE_SECURE_MEM on Windows is bounded by the process working set quota (200 kB by default, 44 collectors)
 * Windows: the CPU count and the timer thread's CPU follow the calling thread's affinity; a process confined to one CPU refuses the internal timer
 * Windows: jent_fips_enabled() reports the system FIPS policy
 * Jitter RNG core: a collector stores the flags it was allocated with; the memory size and hash loop count the library derives from them live in the fields that hold them and grow with the reinitialization count, which now counts every reallocation, a replacement's own startup ladder included. Whether the caller chose the memory size is read off the flags and whether recovery is exhausted off the oversampling rate; the two fields that recorded them are gone
 * Linux on Arm: a cache level neither sysfs nor sysconf reports - none of them on Linux before 4.0, or since 4.12 without cache sizes in the device tree or ACPI; neither glibc nor bionic reports one on arm64 - is the largest the Technical Reference Manual of the core types in /proc/cpuinfo allows, the L3 the largest of the DynamIQ Shared Unit the core is designed for. The table covers the Cortex-A5 to A17, the A32 to X925 and the Neoverse cores, and the Qualcomm part numbers Linux applies Arm errata to. The memory access region was sized by the 256 kB default there, and jent_status reported no cache
