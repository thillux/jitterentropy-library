 ...
 * Linux kernel: every health test failure of a FIPS or NTG.1 instance is logged (rate-limited) with the failing tests, whether intermittent or permanent, the oversampling rate and the instance UUID - including those ending a collector startup, which the interfaces otherwise only report as ENOMEM
