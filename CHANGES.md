 ...
 * Linux on Arm: where neither sysfs nor sysconf reports a cache size - Linux before 4.0, or since 4.12 without cache sizes in the device tree or ACPI; neither glibc nor bionic reports one on arm64 - the cache sizes are the largest the Technical Reference Manual of the core types in /proc/cpuinfo allows. The memory access region was sized by the 256 kB default there, and jent_status reported no cache
