3.7.1-prerelease
 * Health tests: the cutoff tables cover the oversampling rates 1 to 64 instead of 1 to 20. They are indexed by the rate and a build assertion requires an entry for every rate the library accepts, so 20 was as far as JENT_MAX_OSR could be raised; a rate the tables do not reach would run on the cutoffs of the highest one they know, which are stricter than the entropy rate of 1/OSR that rate claims and fail a healthy noise source. The entries up to 20 are unchanged and JENT_MAX_OSR stays 20, so nothing moves unless that tunable is raised. tests/health/cutoffs.py computes the tables for a value beyond 64; what bounds it in turn is 204, where the (256 + safety factor) * OSR time deltas of one output block stop fitting the unsigned short window counters of the repetition count test with memory, and a build assertion now holds JENT_MAX_OSR to it. A rate outside the tables is refused by the health test initialization, which fails the allocation, rather than clamped to their last entry as it was: cutoffs belonging to another rate are not this rate's, and a build that reaches for one has a table to recompute
 * Jitter RNG core: new JENT_MAX_OSR_FIPS and JENT_MAX_OSR_NTG1, both 20, bound the oversampling rate of an instance in the respective compliance mode. What the analysis behind a mode covers is settled when that analysis is made, not by how far the health test tables reach, so the two questions are now two constants: raising JENT_MAX_OSR extends what the library will run at without extending what it claims. An allocation asking for more in a compliance mode is refused, and the reallocation on a health test failure climbs to that mode's ceiling rather than the library's. Both must be at most JENT_MAX_OSR, which is asserted at build time
 * Linux kernel: an `osr` module parameter above what the library accepts refuses the module load with EINVAL rather than being reported as a failed startup, which on a fips=1 kernel panics - a configuration typo is not a verdict on the host
 * Jitter RNG core: every entry point that runs the noise source clears the stack it ran over before it returns - jent_read_entropy(), jent_read_entropy_safe() where a health-test reset gave up, jent_entropy_collector_alloc() and jent_entropy_init*(). Those paths left their own state in the frames they used - the address selector of the memory access loop, the time stamps and deltas of every measurement, the Keccak round temporaries, and the copies a compiler makes of a sample passed by value - on a stack that is neither locked, guard-paged nor kept out of a core dump, completing what the move of the XDRBG generate buffer into struct jent_sha_ctx started. It is done once at the end rather than in the functions that produced the state, and that is the point: between two time stamps every instruction is inside the interval the next delta measures, so a wipe on that path would become part of what the noise source times. Scrubbing the region also reaches the copies the C source cannot name. The startup is the path that needed it most, and not the generate: a generate ends in the conditioning pass, whose frames overwrite the collection frames on the way out, while jent_time_entropy_init() returns straight from the measurement loop and left raw time stamps behind. The scrubbed region stays 4 kB, which covers the deepest of these paths: the first jent_entropy_init_ex() of a process at 2.6 kB - it alone runs the conditioning known answer tests and the GCD analysis on top of the measurement loop - against 1.3 kB for a generate. The paths being wiped have returned before that buffer is allocated, so it does not add to their depth but replaces it, and is the library's peak stack demand on its own: a hosted build wants some 4.5 kB of stack available to those calls, and enlarging the buffer for margin would come straight out of what a caller on a small thread stack has to provide. Hosted builds only - a kernel or baremetal stack already has the properties that make the collector's allocation secure there
 * Tests: add tests/unit/unit-stack-residue.c, which paints a slab of stack, runs each of those entry points over it and checks three things of what comes back: that the wipe left a whole JENT_STACK_SCRUB_LEN of contiguous zero bytes with no non-zero byte among them, that the band below it still holds nothing but paint, and that no raw time stamp survives anywhere. It takes all three - the second is the only one that can notice a path outgrowing JENT_STACK_SCRUB_LEN, the band above it being cleared by the wipe either way. The time source is mocked so every stamp carries a magic nothing else in the process holds, which is what makes a find attributable, and the stamp search covers the entry point's own frame as well: that frame is the one place the wipe cannot reach, no stamp is any frame's legitimate local, and the startup leaves two of them some 90 bytes down. Where the wiped band begins is measured from the snapshot rather than assumed, that boundary being a frame size and frame sizes being what changes under another compiler. It also checks that the block returned to the caller never reaches the stack
 * Documentation: state what JENT_FORCE_SECURE_MEM requires of a Windows caller. The lock quota there is the process working set, whose default minimum of 200 kB is below the 256 kB memory access region of a default collector, so VirtualLock() fails with ERROR_WORKING_SET_QUOTA and the flag turns that into a failed allocation - as it does for JENT_NTG1 and JENT_FORCE_FIPS, which imply it, leaving no compliance-mode collector allocatable until the quota is raised with SetProcessWorkingSetSizeEx(). The man page named RLIMIT_MEMLOCK and the libgcrypt and OpenSSL arenas, and not the one that denies the lock by default on every Windows machine
 ...
 * Health tests: the APT priming of a health-test reallocation keeps its distance from the permanent cutoff. Both APT cutoffs are capped at the window size and the common tables meet there - at an oversampling rate of 14 they are one apart, from 15 on both are the window size - so priming the replacement at its intermittent cutoff made the next time delta equal to the base symbol a permanent APT failure, which ends the instance and panics a fips=1 kernel, on evidence an instance that had not been reallocated needs a whole window of identical symbols for. Where the two cutoffs have met, the replacement now continues from the count the old window actually reached. The reallocation happens for whichever health test failed, so the APT need not have been the one that triggered it
 * Health tests: the RCT with memory primes the replacement of a reallocation at its own intermittent cutoff rather than at the cutoff of the collector it replaces. The two differ, the replacement being allocated at a higher oversampling rate; the RCT states the same for itself, where a mismatched value is outright harmful. The priming remains ineffective for the reason its comment gives
 * Jitter RNG core: jent_read_entropy_safe() remembers a recovery it has given up on. Once the collector is at the maximum oversampling rate the reallocation cannot be attempted again and the health failure is sticky, so every further read generated an output block only to discard it and report the same failure; it is now reported without spending the block. The error and the FIPS failure callback are unchanged
 * Jitter RNG core: jent_read_entropy_safe() rejects a NULL entropy collector with JENT_ERR_EINVAL rather than reading its state first. Same answer as before, when the check inside jent_read_entropy() was what caught it
 * Jitter RNG core: the instance identifier is assigned by jent_entropy_collector_alloc() rather than by the allocation underneath it, which had every health-test reallocation draw a UUID only for jent_health_failure_reset() to overwrite it with the identifier the instance is known by
