...
static long parse_cpu_list(const char *str, unsigned long *cpus, size_t max)
{
	...
	while (*p && *p != '\n') {
		...
		/*
		 * A range up to ULONG_MAX parses without errno; walking it would
		 * spin and overflow the count. Same ceiling as
		 * jent_ncpu_parse_online(), above JENT_MAX_CPUS so a larger
		 * machine is reported as truncated rather than rejected.
		 */
 ...
 * position equals that number only while a group's active CPUs occupy its
 * lowest bits without a gap, which a parked or disabled CPU ends - and
 * numbering by bit position would then name a CPU other than the one --cpu
 * pins to.
 */
